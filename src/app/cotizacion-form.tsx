...
import type { NetlifyIdentityContextValue } from "@/context/netlify-identity-context";
...
export default function CotizacionForm({ identity }: { identity: NetlifyIdentityContextValue }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window === "undefined") return false;
    return window.matchMedia?.("(prefers-color-scheme: dark)")?.matches ?? false;
  });

  // Suscribe a cambios del sistema (solo actualiza estado en el callback)
  useEffect(() => {
  ...
  // Aplica la clase al documento cuando cambia el modo
  useEffect(() => {
  ...
  const [descuentoGlobalFocused, setDescuentoGlobalFocused] = useState(false);
  ...
  const descuentoGlobalComputed = useMemo(() => {
    if (items.length === 0) return "";
    const first = items[0].descuento_pct;
    return items.every((it) => it.descuento_pct === first) ? String(Math.trunc(first)) : "";
  }, [items]);

  const descuentoGlobalValue = descuentoGlobalFocused ? descuentoGlobalDraft : descuentoGlobalComputed;
        ...
        <header className="mb-6 flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-slate-300">
              ...
              {identity.enabled ? " · Acceso con Netlify Identity" : ""}
            </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2">
            {identity.enabled && identity.user && (
              <>
                <span
                  className="hidden max-w-[14rem] truncate text-xs font-medium text-zinc-600 dark:text-zinc-300 sm:inline"
                  title={identity.user.email ?? undefined}
                >
                  {identity.user.email ?? "Sesión"}
                </span>
                <button
                  type="button"
                  onClick={() => void identity.logout()}
                  className="inline-flex h-11 items-center justify-center rounded-xl border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-900 hover:bg-gray-50 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700"
                >
                  Cerrar sesión
                </button>
              </>
            ...
            <div className="flex flex-wrap items-end justify-end gap-4">
              <div className="flex min-w-[10rem] flex-col gap-1 border-l border-zinc-200 pl-4 dark:border-slate-600">
                ...
                <input
                  ...
                  value={descuentoGlobalValue}
                  onChange={(e) => setDescuentoGlobalDraft(e.target.value)}
                  onFocus={() => {
                    setDescuentoGlobalFocused(true);
                    setDescuentoGlobalDraft(descuentoGlobalComputed);
                  }}
                  onBlur={() => {
                    setDescuentoGlobalFocused(false);
                  ...
                  size={Math.min(4, Math.max(2, descuentoGlobalValue.length || 1))}
                  maxLength={4}
                  inputMode="decimal"
